...
import { DirectionEnum, PlaneStateEnum } from '../utils/Enums';
...
export class PlaneState extends Schema {
    ...
    @type("string") currentDirection:DirectionEnum; // direction we are going right now ( while we in cell )
    @type("string") desiredDirection:DirectionEnum; // direction person want to go ( probably not allowed )
    @type("number") currentSpeed: number;  // current speed of plane
    @type("number") takeOffSpeed: number;  // speed to successfully takeoff
    @type("string") state: PlaneStateEnum;  // 'ok', 'dead', 'takeoff'
    @type("string") color: string;  // 'RED','BLUE','GREEN'
    @type("number") subMove: number; // current part of cell we drove

    subStep:number = 0.05; // each cell will require 20 ticket to pass
    decisionFinal = true; // if we can turn plane direction in the next cell
    userCommand: DirectionEnum;

    constructor(x: number, y: number, initialDirection:DirectionEnum, takeOffSpeed: number, color: string) {
        super();
    ...
    askToTurn(requestedDirection: DirectionEnum) {
        this.userCommand = requestedDirection;
    }

    decideAutoTurn(dir: DirectionEnum) {
