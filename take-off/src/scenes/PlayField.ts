...
export class PlayField extends Scene {   
    ...
    _drawPlanes(state: any) {
        ...
        state.planes.entries().forEach(([sessionId,planeSpec]:any)=>{
            this.__drawSinglePlane(fieldX, fieldY, planeSpec);
        });
    }
    __drawSinglePlane(fieldX: integer, fieldY: integer, planeSpec: any) {
        var subModX = 0;
        var subModY = 0;
        var angle = 0;
        ...
        this.add.image(fieldX + (planeSpec.x + subModX) * this.tsize
                      ,fieldY + (planeSpec.y + subModY) * this.tsize
                      , "planesSpriteSheet"
                      , 0).setRotation(angle * Math.PI / 180);
    }
